(function() {
    ...
    function populateCategoryButtons() {
        ...
        for (const category in kanjiData) {
            ...
            categoryButtonsDiv.appendChild(categoryDiv);
        }
    }

    function deleteCategory(category) {
        const confirmDelete = confirm(`Bist du sicher, dass du die Kategorie "${category}" löschen möchtest?`);
        if (confirmDelete) {
            delete kanjiData[category];
            localStorage.setItem('kanjiData', JSON.stringify(kanjiData));
            populateCategoryButtons();
            alert(`Kategorie "${category}" wurde gelöscht.`);
        }
    }
    ...
    window.addEventListener('load', function() {
        ...
        // Event Listener für den 'Zurück zum Startbildschirm'-Button
